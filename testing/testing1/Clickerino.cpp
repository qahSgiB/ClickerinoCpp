					...
					if (line.length() > 0) {
						...
						if (line.at(0) != '#') {
							...
							if (infoType == "v") {
								...
								float y = stof(infos[2]);
								float z = stof(infos[3]);
...
class Engine {
	public:
		...
		void renderObject(Object object, olc::PixelGameEngine pgengine) {
...
class Demo : public olc::PixelGameEngine {
	public:
		...
		bool OnUserUpdate(float fElapsedTime) override {
			...
			float angleRadians = (2*3.14159*angle)/360;
			float direction1 = sin(angleRadians)*cameraDistance;
			float direction2 = cos(angleRadians)*cameraDistance;

			float height = sin((2*3.14159*heightK)/360)*maxHeight;

			vector3d center = {-direction1, -direction2, height};
			vector3d direction = {direction1, direction2, 0.0};
			vector3d view1 = {direction2, -direction1, 0.0};
			vector3d view2 = {0.0, 0.0, -cameraDistance};
			...
			if (update) {
				angle += angleVel*fElapsedTime;
				heightK += heightVel*fElapsedTime;
			}
